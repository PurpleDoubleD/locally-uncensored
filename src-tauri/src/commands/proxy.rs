...
pub async fn fetch_external(url: String) -> Result<String, String> {
    let resp = ssrf_safe_fetch(&url, "fetch_external", Duration::from_secs(60), 10).await?;
...
pub async fn fetch_external_bytes(url: String) -> Result<Vec<u8>, String> {
    let resp = ssrf_safe_fetch(&url, "fetch_external_bytes", Duration::from_secs(300), 10).await?;
