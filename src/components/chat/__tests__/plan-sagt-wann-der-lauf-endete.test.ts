...
describe('die Leiste fragt beide Laufquellen, nicht nur eine', () => {
  ...
  it('holt das Urteil aus run-idle statt sich ein zweites zu bauen', () => {
    expect(src).toMatch(/runStatusFrom\(generating, threadStatus, isRunStopped\(activeConversationId\)\)/)
...
describe('runStatusFrom ist derselbe Spruch wie runStatusOf', () => {
  it('ohne Lauf und ohne Faden: leer', () => {
    expect(runStatusFrom(false, undefined, false)).toBe('idle')
    expect(isActiveCodexStatus(runStatusFrom(false, undefined, false))).toBe(false)
  })
  ...
  it('ein wartender Faden zaehlt als laufend, auch wenn generating schon weg ist', () => {
    ...
    expect(runStatusFrom(false, 'awaiting_approval', false)).toBe('awaiting_approval')
    expect(isActiveCodexStatus(runStatusFrom(false, 'applying', false))).toBe(true)
  })

  it('Stop gedrueckt heisst cancelling, nicht idle', () => {
    expect(runStatusFrom(true, undefined, true)).toBe('cancelling')
  })

  it('ein Faden im Fehler ist kein laufender Faden', () => {
    expect(runStatusFrom(false, 'error', false)).toBe('error')
    expect(isActiveCodexStatus(runStatusFrom(false, 'error', false))).toBe(false)
  })
