    ...
    details: [
      {
        ...
        items: [
          ...
          'The folder you set under Model Storage is read now, not only written to. Every GGUF in it, up to four levels down, appears under Installed and loads from where it lies. It appears there whichever backend is serving your chat: on a machine running Ollama, a GGUF in that folder used to be read from the disk and shown nowhere. Its tile carries a Use button that hands the chat to the LU Engine and says so in one line, and the way back to Ollama or LM Studio is the provider card it always was.',
          'Model Storage says which backend each folder belongs to. It was one field labelled "(auto-detect)" over a paragraph naming all three backends at once, so nobody could tell whose folder he was setting. Three named rows now: the LU Engine folder you set, with the folder that is actually being read spelled out while the field is empty; the LM Studio folder, read only, or the plain sentence that LM Studio is not installed; and Ollama, which keeps its own store and has no folder to set.',
          'On the Mac, a model folder under Desktop, Documents or Downloads says beforehand that macOS will ask once for access to it, instead of letting that dialog arrive out of nowhere on the first scan.',
          ...
          'The Linux packages ask for the libraries the LU Engine links against. The deb and the rpm named the desktop libraries but not libvulkan1 and libgomp1, so on a machine without them the install went through, the engine then died in the loader, and the message blamed your graphics card. The missing library is named now, together with the command that installs it. If you run the AppImage instead, it needs the Vulkan loader (libvulkan1) from your system, because an AppImage cannot carry that one itself.',
          ...
          'Settings shows the port the LU Engine really runs on, the Model Storage folder says when it could not be read or was too big to scan, and a ComfyUI install or repair can be cancelled from Settings and keeps showing its progress while you look at other settings.',
