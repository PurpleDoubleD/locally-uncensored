    ...
    details: [
      {
        ...
        items: [
          ...
          'A chat model you downloaded stays visible as Installed even while the engine is not running, and its tile has a Use button that starts the engine and loads that model, rather than leaving you with a file you cannot reach.',
          'A running LM Studio stays in the model picker after the chat has moved to the LU Engine. Its models keep their own heading in the list, and picking one gives the local slot back to LM Studio with a line saying so, so the way back costs the same one click the way out did.',
