...
export const ONBOARDING_MODELS: OnboardingModel[] = [
  // The single curated "Recommended" pick — a tiny (~400 MB) instruct model
  // that fits on any machine, downloads in seconds, and gives the user a
  // working chat to verify their setup. Marking only ONE model as
  // recommended is intentional: the previous list had 6 large models all
  // tagged "Recommended", which made the badge meaningless and pushed
  // newcomers toward 5–16 GB downloads they didn't ask for. See LU-Aufgaben
  // P4. The pill is hidden in the UI when the user already has installed
  // models (they don't need a starter recommendation any more).
  { name: 'qwen2.5-0.5b', label: 'Qwen 2.5 0.5B (Starter)', description: 'Tiny instant-chat model — 400 MB, runs on anything. Great for first-test.', size: '0.4 GB', vram: '1 GB', vramGB: 1, recommended: true, agent: false, downloadUrl: HF_OB('bartowski/Qwen2.5-0.5B-Instruct-GGUF', 'Qwen2.5-0.5B-Instruct-Q4_K_M.gguf'), filename: 'Qwen2.5-0.5B-Instruct-Q4_K_M.gguf', sizeGB: 0.4 },
  // Uncensored (abliterated) — all GGUF downloads from HuggingFace
  { name: 'llama3.1-8b-abliterated-q5', label: 'Llama 3.1 8B', description: 'Fast & reliable all-rounder', size: '6 GB', vram: '6 GB', vramGB: 6, uncensored: true, agent: true, downloadUrl: HF_OB('bartowski/Meta-Llama-3.1-8B-Instruct-abliterated-GGUF', 'Meta-Llama-3.1-8B-Instruct-abliterated-Q5_K_M.gguf'), filename: 'Meta-Llama-3.1-8B-Instruct-abliterated-Q5_K_M.gguf', sizeGB: 6 },
  ...
  { name: 'llama3.1-8b', label: 'Llama 3.1 8B', description: 'Meta general-purpose model', size: '5 GB', vram: '6 GB', vramGB: 6, agent: true, downloadUrl: HF_OB('bartowski/Meta-Llama-3.1-8B-Instruct-GGUF', 'Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf'), filename: 'Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf', sizeGB: 5 },
  ...
  { name: 'gemma4-e4b', label: 'Gemma 4 E4B', description: 'Google latest, lightweight MoE', size: '5 GB', vram: '4 GB', vramGB: 4, agent: true, downloadUrl: HF_OB('unsloth/gemma-4-E4B-it-GGUF', 'gemma-4-E4B-it-Q4_K_M.gguf'), filename: 'gemma-4-E4B-it-Q4_K_M.gguf', sizeGB: 5 },
  { name: 'gemma4-27b', label: 'Gemma 4 27B', description: 'Google flagship, native tools + vision', size: '16 GB', vram: '16 GB', vramGB: 16, agent: true, downloadUrl: HF_OB('unsloth/gemma-4-27b-it-GGUF', 'gemma-4-27b-it-Q4_K_M.gguf'), filename: 'gemma-4-27b-it-Q4_K_M.gguf', sizeGB: 16 },
  { name: 'qwen3.5-9b', label: 'Qwen 3.5 9B', description: 'Newest Qwen, strong reasoning + coding', size: '6 GB', vram: '8 GB', vramGB: 8, agent: true, downloadUrl: HF_OB('unsloth/Qwen3.5-9B-GGUF', 'Qwen3.5-9B-Q4_K_M.gguf'), filename: 'Qwen3.5-9B-Q4_K_M.gguf', sizeGB: 6 },
  { name: 'qwen3.5-9b-abliterated', label: 'Qwen 3.5 9B', description: 'Uncensored Qwen 3.5, strong reasoning', size: '6 GB', vram: '8 GB', vramGB: 8, uncensored: true, agent: true, downloadUrl: HF_OB('mradermacher/Qwen3.5-9B-abliterated-GGUF', 'Qwen3.5-9B-abliterated.Q4_K_M.gguf'), filename: 'Qwen3.5-9B-abliterated.Q4_K_M.gguf', sizeGB: 6 },
  { name: 'gemma4-27b-abliterated', label: 'Gemma 4 27B', description: 'Google flagship uncensored, tools + vision', size: '16 GB', vram: '16 GB', vramGB: 16, uncensored: true, agent: true, downloadUrl: HF_OB('LiconStudio/Gemma-4-27B-it-abliterated-GGUF', 'gemma-4-27B-it-abliterated-Q4_K_M.gguf'), filename: 'gemma-4-27B-it-abliterated-Q4_K_M.gguf', sizeGB: 16 },
]
