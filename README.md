...
The effort release. A reasoning model now has an effort control beside its Think button, Low, Medium and High with Max on GLM 5.3, and the steps come from the server for each model, so a model that offers none keeps the plain button it always had. GLM 5.3 (Pro) and GLM 5.3 Flash (Hosted) joined the cloud catalogue, and the cloud model list holds one fixed order, because the upstream provider shuffles its own list on every call and a new chat opened on whatever happened to be first. Document Chat works in Cloud mode, with your files indexed on your own machine and only the passages that match your question travelling with the prompt. The built-in engine goes by LU Engine now: it moves to a free port when 8127 is taken, retries once after a start that fails, and a chat model you downloaded stays visible as Installed with a Use button that starts the engine for it. A running LM Studio keeps its own heading in the picker, and one click hands the local slot back to it. Model Storage is read now, not only written to, so every GGUF up to four levels down shows up under Installed, and the three backends got three named rows instead of one field labelled auto-detect. The CivitAI API key has a field again. AMD on Windows is read from the HIP SDK, because the only ROCm probe ran rocm-smi and the Windows SDK does not ship it (#123); that one is researched rather than proven, since there is no RDNA4 card here. The Model Manager also stopped reading system RAM as video memory. AMD cards on Linux report their memory size without ROCm installed, because LU read that size only through rocm-smi, which comes with the ROCm developer packages rather than with the driver, so the card was found and its size was not; that reading has now been measured on a rented AMD Instinct MI325X, and on that card LU also lists it with its name and gfx target (a compute card reports itself as a processing accelerator rather than as graphics, so it used to be missing from the hardware list entirely), installs PyTorch from the ROCm channel it picks, and rendered an image, a video, a song, a 4x upscale and a cutout in Create. Also fixed: the deb and the rpm ask for libvulkan1 and libgomp1 instead of letting the engine die in the loader, the ComfyUI installer proves that the environment it just built can import ComfyUI and Repair can be cancelled, the Coding Agent's working directory can be removed again, the prompt history in Create can be cleared, the side panel folds away with your latest chats on the main screen, Settings shows the port the LU Engine really runs on, Windows error messages arrive in English, a ComfyUI that will not start names the cause instead of sending you into a repair that cannot fix it, every Python step LU starts runs with UTF-8 output so a Windows account name outside the English alphabet stops breaking the install partway through, German phrasing reaches the chat tools again now that the filler word "mal" no longer counts as the command to paint and the German words for the internet are on the list, and the Mac stopped scanning your whole home folder for a ComfyUI it never runs there, which is what made macOS ask for access to Apple Music and the Desktop at first launch. Character training on a Windows machine with more than one GPU no longer dies at the first step over a libuv that the Windows torch wheels do not carry (#121); LU sets USE_LIBUV=0 for every trainer process there, the documented torch workaround, since nobody here has two GPUs to measure it on. The uncensored Qwen 3.8 27B rows in the Model Manager come from OrcaRouter's abliteration now, with OrcaRouter's own Ollama tag beside them, and a gated Hugging Face repo no longer ends in "trying again cannot help": the download says the repo needs an accepted licence and a Hugging Face token, and the token field exists on Windows and Linux too.
