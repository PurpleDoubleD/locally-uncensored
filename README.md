...
**Current release: v2.6.9** (September 2026). Portable-friendly installers for Windows and Linux (no admin rights required), under the short in-app name **LU** by LU Labs. Full history in [Releases](https://github.com/PurpleDoubleD/locally-uncensored/releases) and [CHANGELOG.md](CHANGELOG.md).

### What's new in 2.6.9

- The top bar and the Create toolbar are back to the 2.6.7 layout after feedback on Discord: entries stay where they are, nothing scrolls, nothing is cut off.
- The context size menu in Chat opens upwards when there is no room below it, so every size is reachable again.
- Linux installs from the AUR or unpacked by hand update again: LU fetches the signed AppImage into your home folder, points the start menu entry at it and restarts, and from then on updates install in place.
